...
create index results_data_source on wqp_results(data_source);
...
create index results_analytical_method on wqp_results(analytical_method);
--rollback drop index results_analytical_method;

--changeset drsteini:1IndexesAE
create index results_p_code on wqp_results(p_code);
--rollback drop index results_p_code;

--changeset drsteini:1IndexesAF
create index results_activity on wqp_results(activity);
--rollback drop index results_activity;

--changeset drsteini:1IndexesAG
create index results_characteristic_name on wqp_results(characteristic_name);
--rollback drop index results_characteristic_name;

--changeset drsteini:1IndexesAH
create index results_characteristic_type on wqp_results(characteristic_type);
--rollback drop index results_characteristic_type;

--changeset drsteini:1IndexesAI
create index results_sample_media on wqp_results(sample_media);
--rollback drop index results_sample_media;

--changeset drsteini:1IndexesAJ
create index results_organization on wqp_results(organization);
--rollback drop index results_organization;

--changeset drsteini:1IndexesAK
create index results_site_type on wqp_results(site_type);
...
create index results_country_code on wqp_results(country_code);
--rollback drop index results_country_code;

--changeset drsteini:1IndexesAS
create index results_state_code on wqp_results(state_code);
--rollback drop index results_state_code;

--changeset drsteini:1IndexesAT
create index results_county_code on wqp_results(county_code);
...
create index stations_data_source on stations(data_source);
--rollback drop index stations_data_source;

--changeset drsteini:1IndexesAW
create index stations_organization on stations(organization);
--rollback drop index stations_organization;

--changeset drsteini:1IndexesAX
create index stations_site_type on stations(site_type);
...
create index stations_country_code on stations(country_code);
--rollback drop index stations_country_code;

--changeset drsteini:1IndexesBF
create index stations_state_code on stations(state_code);
--rollback drop index stations_state_code;

--changeset drsteini:1IndexesBG
create index stations_county_code on stations(county_code);
