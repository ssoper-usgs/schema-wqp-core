create or replace package body etl_helper as
    ...
    begin
        ...
        select /*+ parallel(4) */ 
          ...
          from result_ct_sum_swap_' || p_table_suffix || '
        ...
        table_name := dbms_assert.sql_object_name(upper('project_dim_swap_' || p_table_suffix));

        dbms_output.put_line('populating:' || table_name);
        execute immediate 'truncate table ' || table_name;

        execute immediate 'insert /*+ append parallel(4) */  into ' || table_name || q'!
			(data_source_id, code_value, project_dim_value)
		select /*+ parallel(4) */
		       data_source_id,
		       code_value,
		       project_dim_value
		  from (select distinct
		          ...
		          from result_ct_sum_swap_!' || p_table_suffix || q'!,
        ...
        select /*+ parallel(4) */ 
               distinct data_source_id,
                        code_value
          from project_dim_swap_' || p_table_suffix || '
         where project_id is not null';
        commit;
