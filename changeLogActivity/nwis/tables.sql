...
--changeset drsteini:WQP-932-alter-result_ct_sum_swap_nwis-activity_id
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(*) from user_tab_columns where table_name = 'RESULT_CT_SUM_SWAP_NWIS' and column_name = 'ACTIVITY_ID'
alter table result_ct_sum_swap_nwis add activity_id number;
--rollback select 'cannot rollback add on compressed table need manually drop table and re-create using changeset changeLog1/nwis/tables/drsteini:1NwisTablesAE and 1NwisTablesAW manually' from dual;

--changeset drsteini:WQP-932-alter-result_nr_sum_swap_nwis-activity_id
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(*) from user_tab_columns where table_name = 'RESULT_NR_SUM_SWAP_NWIS' and column_name = 'ACTIVITY_ID'
alter table result_nr_sum_swap_nwis add activity_id number;
--rollback select 'cannot rollback add on compressed table need manually drop table and re-create using changeset changeLog1/nwis/tables/drsteini:1NwisTablesAF and 1NwisTablesAX manually' from dual;
