...
--changeset drsteini:WQP-932-alter-result_ct_sum_swap_biodata-activity_id
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(*) from user_tab_columns where table_name = 'RESULT_CT_SUM_SWAP_BIODATA' and column_name = 'ACTIVITY_ID'
alter table result_ct_sum_swap_biodata add activity_id number;
--rollback select 'cannot rollback add on compressed table need manually drop table and re-create using changeset changeLog1/biodata/tables/drsteini:1BiodataTablesAE and 1BiodataTablesAX manually' from dual;

--changeset drsteini:WQP-932-alter-result_nr_sum_swap_biodata-activity_id
--preconditions onFail:MARK_RAN onError:HALT
--precondition-sql-check expectedResult:0 select count(*) from user_tab_columns where table_name = 'RESULT_NR_SUM_SWAP_BIODATA' and column_name = 'ACTIVITY_ID'
alter table result_nr_sum_swap_biodata add activity_id number;
--rollback select 'cannot rollback add on compressed table need manually drop table and re-create using changeset changeLog1/biodata/tables/drsteini:1BiodataTablesAF and 1BiodataTablesAY manually' from dual;
